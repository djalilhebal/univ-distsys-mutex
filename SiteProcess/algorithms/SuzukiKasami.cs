...
{
    /**
     * Suzuki-Kasami (Version finale)
     * #Jeton #JetonRequetes
     *
     * NOTES
     * ------
     * - Message complexity: `{ 0 , n }` (KAITO = n = n - 1 for myself request messages + 1 token message)
     * - Synchronizatin delay: `[ 0 ; 2*t ]`
     *
     * TLDR
     * -----
     * - The difference between the initial version and this (final) one is that it adds,
     *     * OnGotJeton:   `∀ j : nbreqi[j] := max(nbreqi[j], jeton[j])`
     *     * OnGotRequest: `nbreqi[j] = Math.Max(nbreqi[j], nj);`
     */
    public class SuzukiKasami : AbstractAlgo
    {
        ...
        public SuzukiKasami(char siteId, char[] siteIds)
            : base(siteId, siteIds)
        {
            i = siteId;
